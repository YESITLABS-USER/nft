...
const CoupanComponent = ({coupan_type, coupan_discount, coupan_title, coupan_validity,coupan_age, coupan_color, occupied, onClick}) => {
    const colorMapping = {
        orange: orange,
        blue: blue,
        red: red,
  ...
  return (
    <div className="coupan-container" style={{ cursor:"pointer" ,filter: occupied ? "brightness(0.5)" : ""}} onClick={onClick}>
      <img src={colorMapping[coupan_color]} alt="Coupon" className="coupan-image" />
      <div className="coupan-content">
        <div className="coupan-details">
          <div className="coupan-vertical-text" style={{color:'#FF6B00'}}>
            <p className="coupan-description">{coupan_type || "Beverages coupon"}</p>
            <h2 className="coupan-offer">{coupan_discount || "FREE"}</h2>
          </div>
          <div className="coupan-main-text">
            <h2 className="coupan-title" > {coupan_title.length <= 9 ? coupan_title.split(" ").map((word, index) => (
            ...
            <span className="coupan-validity">
              <span style={{color:'#d9d0d0'}}>VALID UNTIL </span> {coupan_validity || "31 DECEMBER 2025"}
            </span>
